...
func Test_pairOfUserAndGroupRepository_RemovePairOfUserAndGroup(t *testing.T) {
	...
	fn := func(t *testing.T, ctx context.Context, ts testService) {
		...
		user1 := testAddUser(t, ctx, ts, owner, "LOGIN_ID_1", "USERNAME_1", "PASSWORD_1")
		// user2 := testAddUser(t, ctx, ts, owner, "LOGIN_ID_2", "USERNAME_2", "PASSWORD_2")
		// user3 := testAddUser(t, ctx, ts, owner, "LOGIN_ID_3", "USERNAME_3", "PASSWORD_2")
		// group1 := testAddUserGroup(t, ctx, ts, owner, "GROUP_KEY_1", "GROUP_NAME_1", "GROUP_DESC_1")
		...
		err = pairOfUserAndGroupRepo.AddPairOfUserAndGroup(ctx, owner, user1.GetUserID(), ownerGroup.UserGroupID)
		require.NoError(t, err)
		{
			userGroups1, err := pairOfUserAndGroupRepo.FindUserGroupsByUserID(ctx, user1, user1.GetUserID())
			require.NoError(t, err)
			assert.Len(t, userGroups1, 1)
		}

		// when
		err = pairOfUserAndGroupRepo.RemovePairOfUserAndGroup(ctx, owner, user1.GetUserID(), ownerGroup.UserGroupID)
		require.NoError(t, err)
		...
		// // when
		// err = pairOfUserAndGroupRepo.AddPairOfUserAndGroup(ctx, user2, user3.GetUserID(), ownerGroup.GetUerGroupID())
		// // then
		// // - user2 can NOT make user3 belong to owner-group
		// assert.ErrorIs(t, err, libdomain.ErrPermissionDenied)

		// // when
		// err = pairOfUserAndGroupRepo.AddPairOfUserAndGroup(ctx, user1, user3.GetUserID(), group1.GetUerGroupID())
		// // - user1 can make sure user3 belong to group1 because user1 belongs to owner-group
		// assert.NoError(t, err)
		// {
		// 	userGroups3, err := pairOfUserAndGroupRepo.FindUserGroupsByUserID(ctx, user1, user3.GetUserID())
		// 	require.NoError(t, err)
